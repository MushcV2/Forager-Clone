...
public class PlayerInventory : MonoBehaviour
{
    ...
    private GameObject SelectedInventoryEmpty(GameObject _itemToVerify)
    {
        ...
        {
            if (inventoryUI.transform.GetChild(i).GetComponent<InventorySlot>().haveItem)
            {
                var _onList = inventoryList.Find(_list => _list.item == _itemToVerify);
                if (_onList != null)
                {
                    _slot = inventoryUI.transform.GetChild(i).gameObject;
                    break;
                }
            }
            else _slot = inventoryUI.transform.GetChild(i).gameObject;
        }
